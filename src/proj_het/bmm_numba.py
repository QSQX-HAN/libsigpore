...
class BMMsNumba(object):
    ...
    def fit(self, data, thrshld_ll=1e-5, min_iters=-1, no_of_runs=1):
        ...
        for run_idx in range(no_of_runs):
            ...
            model = BMMNumba(n_clusters=self.K, run_idx=run_idx)
            model.run_EM(data,
...
class BMMNumba(object):
    ...
    def log_p_x_mu(self, k_ind, n_ind):  # log probability of x given mu
        ...
        return self.log_p_x_mu_pure(u, x, pseudo)

    @staticmethod
    @jit(nopython=True, parallel=True)
    def log_p_x_mu_pure(u, x, pseudo):
        return x * np.log(u+pseudo) + (1-x) * np.log(1-u+pseudo)
    ...
    @staticmethod
    @jit(nopython=True, parallel=True)
    def calc_loglikelihood_pure(N, K, pi, mu, data, pseudo):
        ...
        for n_ind in range(N):
            ...
            for k_ind in range(K):
                ...
                l = np.log(pi[k_ind]) + (x * np.log(u+pseudo) + (1-x) * np.log(1-u+pseudo)).sum()
                ll.append(l)
    ...
    def calc_posterior(self):  # posterior probability of latent variable z_n_k
        ...
        loglikelihood = self.calc_loglikelihood_pure(N, K, pi, mu, data, pseudo)
        posterior = [ np.exp(ll-sp.special.logsumexp(ll)) for ll in loglikelihood ]
    ...
    def update_mu(self):  # analytical step for getting new mu
        ...
        for k_ind in range(self.K):
            ...
            for d_ind in range(self.D):
                num = (self.posterior[:,k_ind] * self.data[:,d_ind]).sum()
                denom = self.posterior[:,k_ind].sum()
...
def main(f_input, f_output, f_output_mu, f_likelihood,
    ...
    model = bmm.fit(data, no_of_runs=p_no_of_runs, 
...
def example1(p_clusters):
    ...
    # --------------------------------
    # Initialize BMM class and Run EM
    # --------------------------------
    model = BMMNumba(n_clusters=n_clusters)
    model.run_EM(x)
